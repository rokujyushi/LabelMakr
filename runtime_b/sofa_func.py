...
def load_sofa_dependencies():
	...
	global SOFA_EXPORTER_CLASS, post_processing, fill_small_gaps, add_SP
	...
	sofa_ap_detector = import_module('modules.AP_detector')
	sofa_g2p = import_module('modules.g2p')
	sofa_train_module = import_module('train')
	sofa_task_class = getattr(sofa_train_module, 'LitForcedAlignmentTask')
	sofa_export_tool_module = import_module('modules.utils.export_tool')
	sofa_post_module = import_module('modules.utils.post_processing')
...
def infer_sofa(ckpt: str,
	...
	try:
		...
		print(f"\nOne or more transcription files are causing issues. Please correct the source text and try again. {e}")
	try:
		predictions = get_AP.process(predictions)
	except TypeError as e:
		print(f"\nError in one or more transcriptions. Please correct the source text and try again.\n\nError Code: {e}\n\n")
	assert post_processing is not None
	predictions = post_processing(predictions)

	# output
	processed_predictions, log = predictions
	assert SOFA_EXPORTER_CLASS is not None
