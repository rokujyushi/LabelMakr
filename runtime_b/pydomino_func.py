...
def resolve_ffprobe_executable():
	ffprobe_path = os.environ.get('LABELMAKR_FFPROBE_EXE')
	if not ffprobe_path:
		raise RuntimeError('LABELMAKR_FFPROBE_EXE is not set. Launch alignment through runtime_b/worker.py.')
...
def resolve_ffmpeg_executable():
	...
	if not ffmpeg_path:
		raise RuntimeError('LABELMAKR_FFMPEG_EXE is not set. Launch alignment through runtime_b/worker.py.')
...
def infer_pydomino(onnx_path, output_format='htk', min_aligned_timeframe=3, corpus_dir='corpus'):
	...
	corpus_path = Path(corpus_dir).resolve() if corpus_dir else CORPUS_ROOT
	try:
